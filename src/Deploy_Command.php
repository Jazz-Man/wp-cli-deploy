 ...
 * The constants should be prefixed with the environment handle which you will use as the first paramater for your
 * desired subcommand. An example configuration for a "dev" environment:
 *
 * ```php
 * <?php
 ...
 *    . "echo '$hipchat_message' | %%abspath%%/hipchat-cli/hipchat_room_message -t 1245678 -r 123456 -f 'WP-Cli
 *    Deploy';"
 *  . "fi;"
 ...
 *
 */
class Deploy_Command extends \WP_CLI_Command
{
    ...
    private function push_db()
    {
        ...
        $runner->add(Helpers::get_rsync($dump_file, "$c->ssh:$c->path/$server_file", $c->port),
        ...
        $runner->add("ssh $c->ssh -p $c->port 'cd $c->path;"." mysql --user=$c->db_user --password=".escapeshellarg($c->db_password)." --host=$c->db_host"." $c->db_name < $server_file'",
          'Deployed the database on server.', 'Failed deploying the db on server.');
    ...
    private function push_uploads()
    {
        ...
        $runner->add(Helpers::get_rsync(// When pushing safe, we push the dir, hence no trailing slash
          "$c->local_uploads/", "$c->ssh:$path", $c->port, true, true, $c->excludes),
          "Synced local uploads to '$path' on '$c->host'.", 'Failed to upload the database to the server');
    ...
    private function push_themes()
    {
        ...
        $runner->add(Helpers::get_rsync(// When pushing safe, we push the dir, hence no trailing slash
          "$c->local_themes/", "$c->ssh:$path", $c->port, true, true, "$c->excludes"),
          "Synced local themes to '$path' on '$c->host'.", 'Failed to upload the database to the server');
    ...
    private function push_plugins()
    {
        ...
        $runner->add(Helpers::get_rsync(// When pushing safe, we push the dir, hence no trailing slash
          "$c->local_plugins/", "$c->ssh:$path", $c->port, true, true, "$c->excludes"),
          "Synced local plugins to '$path' on '$c->host'.", 'Failed to upload the database to the server');
    ...
    private function pull_db()
    {
        ...
        $runner->add("ssh $c->ssh -p $c->port 'mkdir -p $c->path; cd $c->path;"." mysqldump --user=$c->db_user --password=".escapeshellarg($c->db_password)." --host=$c->db_host".' --single-transaction'." --add-drop-table $c->db_name > $server_file'",
          "Dumped the remote database to '$c->path/$server_file' on the server.",
          'Failed dumping the remote database.');

        $runner->add(Helpers::get_rsync("$c->ssh:$c->path/$server_file", "$c->wd/$server_file", $c->port, false,
          false // No delete or compression
        ), "Copied the database from the server to '$c->wd/$server_file'.");

        $runner->add("ssh $c->ssh -p $c->port 'cd $c->path; rm -f $server_file'", 'Deleted the server dump.');

        /** TODO Finalize safe mode. */
        $runner->add(!isset($c->safe_mode), "wp db export $c->bk_path/$c->timestamp.sql",
          "Backed up local database to '$c->bk_path/$c->timestamp.sql'");

        $runner->add("wp db import $c->wd/$server_file", 'Imported the remote database.');

        $runner->add(($c->siteurl != $c->url), "wp search-replace --all-tables $c->url $c->siteurl",
          "Replaced '$c->url' with '$c->siteurl' on the imported database.");

        $runner->add(($c->abspath != $c->wp), "wp search-replace --all-tables $c->wp $c->abspath",
    ...
    private static function pull_uploads()
    {
        ...
        /** TODO Finalize safe mode. */
        $runner->add(isset($c->safe_mode), "cp -rf $c->local_uploads $c->bk_path/uploads_$c->timestamp",
          'Backed up local uploads.');

        $runner->add(Helpers::get_rsync("$c->ssh:$c->uploads/", $c->local_uploads, $c->port, true, true, $c->excludes),
    ...
    private static function pull_themes()
    {
        ...
        /** TODO Finalize safe mode. */
        $runner->add(isset($c->safe_mode), "cp -rf $c->local_themes $c->bk_path/themes_$c->timestamp",
          'Backed up local themes.');

        $runner->add(Helpers::get_rsync("$c->ssh:$c->themes/", $c->local_themes, $c->port, true, true, $c->excludes),
    ...
    private static function pull_plugins()
    {
        ...
        /** TODO Finalize safe mode. */
        $runner->add(isset($c->safe_mode), "cp -rf $c->local_plugins $c->bk_path/plugins_$c->timestamp",
          'Backed up local plugins.');

        $runner->add(Helpers::get_rsync("$c->ssh:$c->plugins/", $c->local_plugins, $c->port, true, true, $c->excludes),
    ...
    private static function dump_db($args = [])
    {
        ...
        $runner->add(($c->abspath != $c->wp) || ($c->url != $c->siteurl), "wp db export $c->tmp",
          "Exported a local backup of the database to '$c->tmp'.");

        $runner->add(($c->siteurl != $c->url), "wp search-replace --all-tables $c->siteurl $c->url",
          "Replaced '$c->siteurl' with '$c->url' in local database.");

        $runner->add(($c->abspath != $c->wp), "wp search-replace --all-tables $c->abspath $c->wp",
          "Replaced '$c->abspath' with with '$c->wp' in local database.");

        $runner->add("wp db export $path", "Dumped the database to '$path'.");

        $runner->add(($c->abspath != $c->wp) || ($c->url != $c->siteurl), "wp db import $c->tmp",
          'Imported the local backup.');

        $runner->add(($c->abspath != $c->wp) || ($c->url != $c->siteurl), "rm -f $c->tmp", 'Cleaned up.');
    ...
    private static function sanitize_args($command, $args, $assoc_args = null)
    {
        ...
        self::$runner = new Command_Runner($verbosity);
        ...
        /** Create paths. */
        Command_Runner::get_result('mkdir -p '.self::$config->tmp_path.';');
        Command_Runner::get_result('mkdir -p '.self::$config->bk_path.';');
    ...
    private static function expand($config, $constants, $command, $what)
    {
        ...
        $data = [
          ...
          'hostname'      => Command_Runner::get_result('hostname'),
          'local_uploads' => call_user_func(function () {
              $uploads_dir = wp_upload_dir();

              return untrailingslashit(Command_Runner::get_result("cd {$uploads_dir['basedir']}; pwd -P;"));
          }),
          'local_themes'  => call_user_func(function () {
              $themes_dir = get_theme_root();

              return untrailingslashit(Command_Runner::get_result("cd {$themes_dir}; pwd -P;"));
          }),
          'local_plugins' => call_user_func(function () {
              $plugins_dir = WP_PLUGIN_DIR; // TODO: get the plugin directory in a better manner

              return untrailingslashit(Command_Runner::get_result("cd {$plugins_dir}; pwd -P;"));
          }),
          'siteurl'       => untrailingslashit(Helpers::trim_url(get_option('siteurl'), true)),
          'object'        => (object)array_map('untrailingslashit', $constants),
        ];

        foreach ($config as &$item) {
            $item = Helpers::unplaceholdit($item, array_merge(/** This ensures we can have dependecies. */
              $config, $data));
        }

        if (isset($constants['post_hook'])) {
            $config['post_hook'] = Helpers::unplaceholdit($config['post_hook'], array_merge($config, $data));
        }
    ...
    public function run_post_hook()
    {
        ...
        if (isset(self::$config->post_hook)) {
            $result = Command_Runner::get_result(self::$config->post_hook);
            if (!empty($result)) {
                var_dump($result);
            }
    ...
    {
        $doge = ['wow', 'many', 'such', 'so'];
        $words = ['finish', 'done', 'end', 'deploy'];
        WP_CLI::line('');
