...
class Helpers
{
     ...
     * @param bool $delete
     * @param bool $compress
     * @param bool $user_excludes
     *
     * @return mixed
     */
    public static function get_rsync($source, $dest, $port, $delete = true, $compress = true, $user_excludes = false)
    {
        ...
        $user_excludes = $user_excludes ? explode(':', (string)$user_excludes) : [];

        $exclude = array_merge($exclude, $user_excludes);

        $rsync = self::unplaceholdit(/** The command template. */
          ...
          [
            ...
            'port'     => $port,
            'dest'     => $dest,
            'exclude'  => '--exclude '.implode(' --exclude ', array_map('escapeshellarg', $exclude)),
          ]);
     ...
     * @return mixed
     */
    public static function unplaceholdit($template, $content, $object_key = 'object')
    {
        ...
        foreach ($matches as $match) {
            ...
            if ($object && isset($object[$match[1]])) {
                $replaces[] = $object[$match[1]];
            } elseif (isset($content[$match[1]])) {
                $replaces[] = $content[$match[1]];
            } else {
                $replaces[] = $match[0];
            }
        }
     ...
     * @return string
     */
    public static function trim_url($url, $path = false)
    {
        ...
        $domain = preg_replace('/^www\./', '',
            $url_parts['host']).(!empty($url_parts['port']) ? ':'.$url_parts['port'] : '');
